...
class NNClassifier(object):
    ...
    def read_data(self):
        ...
        Y = []
        for i in range(10):
            X.append(np.array(np.fromfile('vals%d.dat' % i, dtype=np.uint16).reshape((-1, 8))))
    ...
    def train(self, X, Y):
        ...
        if HAVE_SK and self.X.shape[0] >= K * SUBSAMPLE:
            ...
            print(self.X[::SUBSAMPLE].shape, self.Y[::SUBSAMPLE].reshape(-1,1).shape)
            self.nn.fit(self.X[::SUBSAMPLE], self.Y[::SUBSAMPLE].reshape(-1,1))
        else:
            self.nn = None
