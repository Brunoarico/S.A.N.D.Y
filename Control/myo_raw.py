...
import threading
import time
from math import fabs, atan2, copysign, asin, pi, degrees, sqrt
...
def to_euler(quaternion):
    ...
    euler_angles[2] = degrees(atan2(sinr_cosp, cosr_cosp))
    ...
    if (fabs(sinp) >= 1):
        ...
        pitch = asin(sinp)
    euler_angles[1] = degrees(pitch)

    # yaw (z-axis rotation)
    siny_cosp = 2 * (w * z + x * y)
    cosy_cosp = 1 - 2 * (y * y + z * z)
    euler_angles[0] = degrees(atan2(siny_cosp, cosy_cosp))
